...
function DetailsView({
  ...
  return (
    <article className="col-start-3 col-end-4 row-start-2 row-end-4 flex flex-col items-center gap-y-3 bg-white px-8 py-5">
      <h3 className="text-2xl">채점 영역별 점수</h3>
      <table className="w-full flex-1 whitespace-nowrap">
        <thead>
          <tr className="bg-primary-50">
            <th colSpan={2}>채점 영역</th>
            <th className="h-9 border-l border-white px-1">점수</th>
            <th className="h-9 border-l border-white px-1">총점</th>
          </tr>
        </thead>
        <tbody className="w-full text-center">
          {dataList.map((category, idx) =>
            category.detail.map((value, subIdx) => (
              ...
              >
                ...
                <td className="h-9 whitespace-nowrap border-r border-white">
                  {value.title}
                </td>
          ...
          <tr className="border-b border-white bg-gray-300/30">
            <td className="h-9 whitespace-nowrap border-r border-white text-center">
              종합
            </td>
            <td className="border-r border-white" />
            <td className="border-r border-white" />
            <td className="whitespace-nowrap text-center text-lg">
              {total.score}
